...
with webdriver.Chrome() as browser:
    ...
    for cook in cookies:
        ...
        browser.refresh()
        ar=[]
        info = browser.find_element(By.ID,"info")
        try:
            age = int(info.find_element(By.ID,'age').text.split()[1])
        except:
            pass
        lang = browser.find_elements(By.TAG_NAME,'li')
        for i in lang:
            ar.append(i.text)
        ar=len(ar)
        d[cook['name']]=(age,ar)

# Теперь переменная d доступна
    st = max(d,key=lambda x:(-d[x][0],-d[x][1]))
